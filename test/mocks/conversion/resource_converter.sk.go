...
type MockCustomTypeUpConverter interface {
	FromV1Alpha1ToV1(src *v1alpha1.MockCustomType) *v1.MockCustomType
	FromV1ToV2Alpha1(src *v1.MockCustomType) *v2alpha1.MockCustomType
}

type MockCustomTypeDownConverter interface {
	FromV1ToV1Alpha1(src *v1.MockCustomType) *v1alpha1.MockCustomType
	FromV2Alpha1ToV1(src *v2alpha1.MockCustomType) *v1.MockCustomType
}

type mockCustomTypeConverter struct {
	upConverter   MockCustomTypeUpConverter
	downConverter MockCustomTypeDownConverter
}

func NewMockCustomTypeConverter(u MockCustomTypeUpConverter, d MockCustomTypeDownConverter) crd.Converter {
	return &mockCustomTypeConverter{
		upConverter:   u,
		downConverter: d,
	}
}

func (c *mockCustomTypeConverter) Convert(src, dst crd.SoloKitCrd) error {
	srcVersion, err := kubeapi.ParseVersion(src.GetObjectKind().GroupVersionKind().Version)
	if err != nil {
		return err
	}
...
func (c *mockCustomTypeConverter) convertUp(src, dst crd.SoloKitCrd) error {
	if src.GetObjectKind().GroupVersionKind().Version == dst.GetObjectKind().GroupVersionKind().Version {
		return crd.Copy(src, dst)
	}

	switch t := src.(type) {
	case *v1alpha1.MockCustomType:
		return c.convertUp(c.upConverter.FromV1Alpha1ToV1(t), dst)
	case *v1.MockCustomType:
		return c.convertUp(c.upConverter.FromV1ToV2Alpha1(t), dst)
	}
	return errors.New("unrecognized source type, this should never happen")
}

func (c *mockCustomTypeConverter) convertDown(src, dst crd.SoloKitCrd) error {
	if src.GetObjectKind().GroupVersionKind().Version == dst.GetObjectKind().GroupVersionKind().Version {
		return crd.Copy(src, dst)
	}
