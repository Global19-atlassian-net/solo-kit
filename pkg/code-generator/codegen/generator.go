...
func GenerateFiles(project *model.Project, skipOutOfPackageFiles, skipGeneratedTests, genKubeTypes bool, genJsonSchema bool) (code_generator.Files, error) {
	...
	jsonSchemaGenerator, err := jsonschema.NewGenerator(project.Request, nil)
	if err != nil {
		return nil, err
	}
	...
	for _, res := range project.Resources {
		...
		if genJsonSchema {
			jsonSchemaBytes, err := jsonSchemaGenerator.Convert(res)
			res.JsonSchema = string(jsonSchemaBytes)

			crdJsonSchemaFiles, err := generateJsonSchemaFilesForResource(res)
			if err != nil {
				return nil, err
			}
			files = append(files, crdJsonSchemaFiles...)
		}
...
func generateJsonSchemaFilesForResource(resource *model.Resource) (code_generator.Files, error) {
	var v code_generator.Files
	for suffix, tmpl := range map[string]*template.Template{
