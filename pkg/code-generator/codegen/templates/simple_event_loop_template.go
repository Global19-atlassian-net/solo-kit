...
var SimpleEventLoopTemplate = template.Must(template.New("simple_event_loop").Funcs(Funcs).Parse(`package {{ .Project.VersionConfig.Version }}
...
func (el *{{ lower_camel .GoName }}SimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	ctx = contextutils.WithLogger(ctx, "{{ .Project.VersionConfig.Version }}.event_loop")
	...
	go errutils.AggregateErrs(ctx, errs, emitterErrs, "{{ .Project.VersionConfig.Version }}.emitter errors")
	go func() {
